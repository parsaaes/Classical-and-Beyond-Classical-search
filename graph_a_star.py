...
def ASTAR(problem, h):
    ...
    while True:
       ...
       for action in problem.actions(node.state):
           g = node.g + problem.path_cost(node.state,problem.transition(node.state,action))
           child = node.child_node(problem,action,g + h[problem.transition(node.state,action)],g)
