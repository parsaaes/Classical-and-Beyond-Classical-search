...
def GBFS(problem, h):
    ...
    while True:
       ...
       if problem.goal_test(node.state):
           return (node,(visited,expanded_nodes,maximum_memory,node.g))
       explored.add(node.state)
       for action in problem.actions(node.state):
           g = node.g + problem.path_cost(node.state,problem.transition(node.state,action)) # just for calculating path cost
           child = node.child_node(problem,action, h[problem.transition(node.state,action)], g)
...
def main():
    ...
    node, details = GBFS(romania_prob,pg.romania_huristic())
    if node is not None:
        node.print_path()
        print(details)
    return
